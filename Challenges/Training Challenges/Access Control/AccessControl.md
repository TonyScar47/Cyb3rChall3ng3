...
- **SUID**: minimize SUID binaries (`find / -perm -4000`), strip the bit where it isn't needed. An editor or tool that doesn't need to elevate shouldn't carry it.
- **PATH / commands**: in privileged programs always use absolute paths (`/bin/echo`) and reset `PATH` to a known value before any `system()`. Better still, don't use `system()`: use `execve` with a controlled environment.
- **Env injection**: never concatenate environment variables into shell strings. Validate/allowlist, or pass arguments without a shell in between.
- **Symlink/TOCTOU**: open the file once and operate on the file descriptor (`open` then `fstat`, not `access` then `open`); use `O_NOFOLLOW` to reject symlinks; sticky bit on world-writable dirs (`/tmp`) to stop other people's files being swapped. TOCTOU is closed by eliminating the window between check and use: a single syscall that does both.